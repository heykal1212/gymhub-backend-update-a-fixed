...
async function main() {
  await prisma.membershipTier.create({ data: { name: 'Basic', priceMonth: 800, benefits: ['Gym floor'], tierLevel: 0 } });
  await prisma.membershipTier.create({ data: { name: 'Plus', priceMonth: 1200, benefits: ['Gym floor','Classes'], tierLevel: 1 } });
  await prisma.membershipTier.create({ data: { name: 'Pro', priceMonth: 1800, benefits: ['All access','PT discount'], tierLevel: 2 } });
